...
function parseCSV(text: string): ImportResult {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    valid.push({ name, phone, address, area, gstin });
  }

  return { total: lines.length - 1, valid, errors };
}

const SAMPLE_CSV = `name,phone,address,area,gstin
Amit Patel,+91 99887 11234,15 MG Road Pune,East Zone,27AABCP1234A1Z5
Priya Electronics,+91 98123 45678,22 Station Rd Mumbai,West Zone,`;
                  ...
                  <table className="w-full text-xs">
                    <thead>
                       <tr className="border-b border-border text-left text-muted-foreground">
                         <th className="pb-1">Name</th><th className="pb-1">Phone</th><th className="pb-1">Area</th><th className="pb-1">Address</th>
                       </tr>
                     </thead>
                     <tbody>
                       {result.valid.slice(0, 10).map((c, i) => (
                          <tr key={i} className="border-b border-border last:border-0">
                            <td className="py-1">{c.name}</td><td className="py-1">{c.phone}</td><td className="py-1">{c.area}</td><td className="py-1">{c.address}</td>
                          </tr>
                        ))}
